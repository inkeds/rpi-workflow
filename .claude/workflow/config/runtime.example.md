...
**类型**：`boolean`  
**默认值**：`false`  
**说明**：关闭任务前是否要求适用的设计同步证据

**作用**：
- 开启后，行为或契约变化必须同步对应 Spec/Capability/Invariant；局部修复可用“不需要设计更新”的明确证据满足
- 关闭后，不单独强制旧式 Spec Sync，但功能任务仍受 reconciliation 与 strict mode 约束

**适用场景**：
- `true`：需要显式证明设计维护结论的项目
- `false`：快速原型、临时任务
