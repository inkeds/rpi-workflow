...
单文件协议与完整 RPI 共享事实、能力、不变量、变更、决策和对账等治理概念，但执行层不同：单文件依赖项目自身文档和 Agent 遵循自然语言协议；完整 RPI 额外提供 `CHG-*`、`DEC-*`、`CNF-*`、Schema、Hook、Gate、锁和事务。两条支线保持概念兼容，不要求目录、命令或自动化能力逐项一致。
...
- **Systematic Debugging**：复现、边界取证、单一根因假设、回归测试和架构升级判断。
- **Code Reviewing**：围绕 Spec、正确性、安全、迁移和测试证据执行 A2A/人工评审闭环。
- **Quality Gates**：按阶段执行测试、规范、架构、UX 和模块联动检查；UX Skill 同时覆盖视觉方向、可访问性、响应式和 React/Next.js 高影响性能规则。
...
| 项目配置扩展点 | `.codex/config.toml`（默认不覆盖模型、审批和沙箱） | `.claude/settings.json` |
| Skills | `.agents/skills/` | `.claude/skills/` |
| 生命周期 Hooks | `.codex/hooks.json` | `.claude/settings.json` |
...
| 裁决设计冲突 | `rpi.sh change resolve <CHG-ID> <CNF-ID> --resolution <方案> --evidence "<依据>"` |
| 刷新变更基线 | `rpi.sh change rebase <CHG-ID> --evidence "<依据>"` |
