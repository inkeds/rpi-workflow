...
1. **产品体验优先**：先明确用户任务、状态与恢复路径，再决定组件形式
2. **视觉表达明确**：继承项目语言并记录有意图的视觉取舍，避免模板化生成
3. **可访问与可验证**：键盘、焦点、语义、对比度和状态矩阵进入验收
4. **性能受预算约束**：React 数据流、Bundle、渲染和媒体资源有可测边界
5. **持续对账**：功能性交互变化同步 Change、Spec、Capability/Invariant 与证据
...
3. 明确体验目标、视觉方向、组件边界、状态矩阵、可访问性和性能预算
...
**检查内容**：
- UX 规范骨架、禁止行为和标杆引用是否完整
- 当前任务是否覆盖加载、空、错误、权限和恢复状态
- 组件复用、表单反馈、可访问性与性能证据是否与 Spec 一致
- 功能性交互变化是否完成设计实现对账
...
**检查逻辑**：
1. 确定性检查规范章节、任务绑定和可定位证据
2. 结合当前变更与标杆模块进行人工或 Agent 评审
3. 运行项目自己的可访问性、交互与性能测试
4. 输出违反明确项目约束的问题；不声称仅靠静态扫描理解全部 UX 语义
...
1. src/views/user/index.vue:45
   问题：提交失败后只显示通用 Toast，字段错误不可定位
   规范：可恢复错误必须关联到对应字段或操作，并保留用户输入
   建议：参考 reference-module.md 的错误恢复模式

2. src/views/order/index.vue:78
   问题：自定义点击容器无法通过键盘聚焦和触发
   规范：交互控件必须使用语义元素并具备可见焦点
   建议：使用项目 Button 组件并补充键盘测试
...
**错误做法**：
```
直接让 AI 生成页面 → 只描述组件外形、不定义用户任务与完整状态 → 返工
```

**正确做法**：
```
1. 补全 ux-spec.md，定义任务、视觉方向、状态、可访问性与性能预算
2. 开发标杆模块，验证 UX 规范
3. 后续模块参考标杆模块，并记录必要差异
4. 通过确定性检查、项目测试和设计实现对账
```
...
**错误做法**：
```
只运行结构检查 → 把“文件完整”误判为“体验正确”
```

**正确做法**：
```
/rpi-check ux 检查基线 → 项目测试验证行为 → 人工或 Agent 评审体验语义 → reconciliation 收口
```
...
A: 不是。简单且无关键交互的展示页面可以只使用 L0 Spec；复杂任务流、品牌体验、表单密集或性能敏感前端更适合启用本模块。

### Q: 标杆模块必须是用户管理吗？

A: 不是。应选择最能代表核心用户任务、组件语言和状态处理的已验证页面，而不是固定选择 CRUD 页面。

### Q: UX 规范定义后可以修改吗？

A: 可以。局部修复更新证据即可；用户流程、交互契约或设计不变量变化必须关联 Change，并同步 Spec、标杆和受影响页面。
