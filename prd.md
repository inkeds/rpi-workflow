...
RPI 面向可能只有模糊创意、复制功能描述或跨平台功能拼接的普通用户。用户原始输入只是产品素材，不是自动生效的需求。RPI 必须先完成来源保真、冲突分析、能力验证和产品取舍，再形成正式 PRD/Spec。

RPI 的工程内核保持 Spec 驱动与 TDD 驱动：产品事实形成前采用证据收敛；正式实现开始后，所有代码必须绑定 Spec，并提供 Red/Green、契约测试、E2E 或 AI Eval 证据。

RPI Core 平台无关，Claude Code CLI 与 Codex CLI 使用各自 Adapter，不能将任一平台的配置、Hook JSON 或本地记忆作为共享事实源。

产品方向流程必须允许生成 0～3 个方向，也允许“当前不能形成可信方向”或“应拆成多个产品”。方向推荐不使用综合百分制，必须同时给出推荐理由、反对理由、关键未知和下一验证实验。用户选择只进入 `selected`，验证后才能晋升为当前产品事实。

AI Eval 提供协议与领域模板，而不是万能质量数据集。第一批模板覆盖结构化提取、基于来源生成和 Agent 工具调用；高风险发布不得只依赖 LLM-as-Judge。模型、Prompt、工具 Schema 或数据集版本变化均应触发多维回归比较。

Codex/Claude 兼容必须区分静态配置和运行时能力。能力状态包括 configured/verified/stale/missing，并根据运行 profile 显式报告治理降级；不得读取或绕过 CLI 私有信任机制。
...
RPI Workflow 是一个面向普通创意者、同时兼容 Claude Code 与 Codex CLI 的工程化 AI 产品定义和开发框架，核心目标是把混乱创意转化为可信规范，并把 AI 编码从“可用但不稳定”升级为“可控、可追溯、可审计、可复现”。
...
| 阶段 | 产品目标 | 工程目标 | 禁止项 |
|------|----------|----------|--------|
| M-1 Explore | 找到可信方向 | 原型与能力验证，实验代码隔离 | 把推断或实验代码直接当成正式事实/实现 |
| M0 Validate | 验证核心价值 | 核心 Spec、TDD 与 Eval | 过早扩张和过度设计 |
| M1 Stabilize | 形成稳定产品 | 契约、异常、集成与回归 | 未经反馈验证扩张 Must |
| M2 Operate | 支撑长期运营 | 安全、可观测、审计、容量与成本治理 | 无运营证据的规模化设计 |

阶段只表达默认投入方向，风险决定最低治理标准。`Vibe:Spec` 比例可作为沟通参考，但不得作为可计算的质量指标。
...
- `init → deepen → spec expand` 三段必须共享同一领域画像源，避免前后阶段出现不同的 Must/Won't 结论。
- `discovery.md / epic.md / spec.md / milestones.md / tasks.md` 必须是业务化产物，不能回落到“核心记录/一线业务操作人员”这类通用模板占位内容。
- 对应用类项目，M0 的“完整可用”定义是：仅围绕已选 Must 链路提供完整 UX 与可回归能力，而不是覆盖所有未来功能。
...
- `/rpi-init` 在初始化阶段立即生成：
  - 业务化 MVP skeleton
  - 第一版业务化 `discovery.md / epic.md / spec.md / milestones.md / tasks.md`
  - A/B/C 业务段摘要与链路候选池
- `/rpi-init deepen` 生成：
  - 4 阶段画布（S1-S4）
  - 链路候选池（链路 ID + 优先级 + 置信度）
  - A/B/C 业务段选择（S0 / S0+S1 / S0+S1+S2）
  - M-1/M0/M1/M2 阶段扩展策略（探索验证 / 价值验证 / 稳定产品 / 长期运营）
  - 覆盖率与不确定性预算
...
- `/rpi-spec expand` 在确认后自动执行：
  - 使用与 `/rpi-init` 相同的业务画像源重写 `discovery/epic/spec/milestones/tasks`
...
- 初始化/深化/规范展开三个阶段的输出必须前后一致：
  - A/B/C 摘要中的 Must/Won't 与 `discovery.md` 一致
  - `discovery.md` 与 `tasks.md` 的链路描述一致
  - `portable contract` 与当前 `discovery/spec/tasks` 一致
...
- `PostToolUse` 必须兼容 Claude Code CLI 当前 Bash payload 的多种返回形态：
  - 优先读取显式 `exit_code/exitCode/returnCode`
  - 其次读取 `success/status/is_error`
  - 再尝试读取 `transcript_path + tool_use_id` 对应的 `tool_result.is_error`
  - 最后回退到 `tool_response.stdout/stderr/interrupted` 的兼容推断
- 设计目标：
  - 不能把成功 Bash 误记为 `exit_code=1`
  - 不能因为 Hook payload 字段缺失而让 `events.jsonl`、自治预算或门控统计失真
...
- 初始化后（执行初始化创建步骤起，例如 `/rpi-init <idea>` 或 `rpi.sh init setup ...`）：
  ...
  - 其中 `specs/l0/discovery.md` 是范围事实源，`specs/l0/spec.md` 是契约事实源，`specs/l0/tasks.md` 是实现事实源。
...
### 5.21 单文件协议与完整 RPI 的边界

- 两条支线共享事实、Capability、Invariant、Change、Decision 和设计—实现对账等稳定概念。
- 单文件协议依赖项目自身文档和 Agent 遵循自然语言规则；完整 RPI 才提供 `CHG-*`、`DEC-*`、`CNF-*`、Schema、Hook、Gate、锁和事务。
- `CNF-*` 只是确定性检测产生的审查候选，不是完整语义冲突的自动证明；用户或评审必须提供裁决依据。
- `change rebase` 只确认已裁决的设计成为新基线，不等于批准变更，也不能替代 Spec、注册表、迁移和测试更新。
- RPI 不要求单文件协议复制其目录、命令或自动化实现；单文件也不应被描述为具备 RPI 的强制门控能力。
...
4. `.rpi-outfile/state/portable/evidence.latest.json`（最新执行证据）
5. `.rpi-outfile/state/agent-review/review_card.latest.json`（标准化裁决卡）
...
- 跑通 `/rpi-init → /rpi-init deepen → /rpi-spec expand → /rpi-task start → /rpi-gates run → /rpi-task close`
